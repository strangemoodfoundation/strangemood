...
async function purchaseWithoutCashier(args: {
  ...
  let [charterTreasuryPublicKey, charterTreasuryBump] = await pda.treasury(
    charterInfo.account.charter,
    charterInfo.publicKey,
    deposit.mint
  );

  let charterTreasury = await args.program.account.charterTreasury.fetch(
  ...
  let ix = await args.program.methods
    ...
    .accounts({
      ...
      charterTreasury: charterTreasuryPublicKey,
      charterTreasuryDeposit: charterTreasury.deposit,
